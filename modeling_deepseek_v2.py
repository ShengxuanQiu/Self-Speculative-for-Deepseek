...
try:
    ...
    # Fallback for older Transformers: act as a no-op context manager.
    @contextmanager
...
class DeepseekV2RotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: DeepseekV2Config, device=None):
        ...
        # Fallback for configs without rope_parameters
...
class DeepseekV2Attention(nn.Module):
    ...
    def __init__(self, config: DeepseekV2Config, layer_idx: Optional[int] = None):
        ...
        # Fallback if config lacks head_dim
        self.head_dim = getattr(config, "head_dim", config.hidden_size // config.num_attention_heads)
        self.max_position_embeddings = config.max_position_embeddings

        self.q_lora_rank = getattr(config, "q_lora_rank", None)
        self.qk_rope_head_dim = getattr(config, "qk_rope_head_dim", self.head_dim)
        self.kv_lora_rank = getattr(config, "kv_lora_rank", self.head_dim)
        self.v_head_dim = getattr(config, "v_head_dim", self.head_dim)
        self.qk_nope_head_dim = getattr(config, "qk_nope_head_dim", 0)
